...
const MONGODB_URI =
  process.env.MONGODB_URI || "mongodb://localhost:27017/lostandfound";
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    // Generate token
    const token = jwt.sign({ username: usernameField }, JWT_SECRET, {
      expiresIn: "24h",
    });
...
app.post("/api/items", authenticateToken, async (req, res) => {
  try {
    // Expecting frontend to send: { title, desc, type, location }
    const { title, desc, type, location } = req.body;
    ...
    const newItem = new Item({
      title,
      desc,
      type: type.toUpperCase(), // force LOST/FOUND
      location,
...
app.post("/api/items/:id/claim", authenticateToken, async (req, res) => {
  try {
    ...
    return res.json({ message: "Item claimed successfully", item: updatedItem });
  } catch (error) {
